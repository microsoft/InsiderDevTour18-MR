...
-	Next, expand the Publishing Settings and scroll to the bottom where the capabilities are, note this is where we assign certain capabilities for the app, make sure Microphone is checked
...
-	E**xpand the RoomPrefab object in the Hierarchy panel, scroll down to the Floor object of the room, drag and drop the Floor object into the slot in the Inspector panel for Floor Quad**
...
- The HololensManager prefab has a few components to handle the unique functionality for HoloLens. 
- **Expand the HololensManager in the Hierarchy panel.** 
- First, let's inspect the RoomPlaceholder. In here it contains the Drone and DroneTarget positioned in world space properly and not as children of the entire rooom. 
...
- Lastly, there is the VoiceCommandsManager. This object contains two toolkit components to define voice command keywords and then attaches action calls to those keywords. 
